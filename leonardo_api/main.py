...
# ---------------------------------------------------------------------------
# Phase 19/20/21: 起動時に必須の環境変数を検証
# 未設定のままデプロイすると、初回 PIN 認証時に 500 エラーが発生する。
# 起動時に検出してデプロイを失敗させる方が安全。
# ---------------------------------------------------------------------------
import os as _os

_REQUIRED_ENV_VARS = [
    "SETUP_PIN_MASTER_SECRET",
]

_missing = [v for v in _REQUIRED_ENV_VARS if not _os.environ.get(v)]
if _missing:
    raise RuntimeError(
        f"必須の環境変数が未設定です: {', '.join(_missing)}. "
        "Render の Environment 設定を確認してください。"
    )
